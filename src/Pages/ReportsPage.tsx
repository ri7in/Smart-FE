...
const ReportsPage: React.FC = () => {
  ...
  const generatePDF = (reportData: ReportData) => {
     console.log("Generating PDF with data:", reportData); // Add this line
    const doc = new jsPDF();
  ...
  const handleGenerateReport = () => {
    const reportData: ReportData = {
      ...
      recycledWaste: 600,
      organicWaste: 700,
      nonRecyclableWaste: 200,
      recyclingRate: 40,
      carbonReduction: 2.5,
      topAreas: [
        { name: 'Downtown', rate: 95 },
        { name: 'Suburbs', rate: 88 },
        { name: 'Industrial Zone', rate: 82 },
      ],
      improvementAreas: [
        { name: 'Old Town', rate: 65 },
        { name: 'University District', rate: 70 },
      ],
    };
  ...
  const handleDownloadExistingReport = (title: string, date: string) => {
    const reportData: ReportData = {
      type: title,
      dateRange: date,
      totalWaste: Math.floor(Math.random() * 1000) + 500,
      recycledWaste: Math.floor(Math.random() * 500) + 200,
      organicWaste: Math.floor(Math.random() * 500) + 200,
      nonRecyclableWaste: Math.floor(Math.random() * 200) + 50,
      recyclingRate: Math.floor(Math.random() * 30) + 30,
      carbonReduction: parseFloat((Math.random() * 3 + 1).toFixed(1)),
      topAreas: [
        { name: 'Area A', rate: Math.floor(Math.random() * 20) + 80 },
        { name: 'Area B', rate: Math.floor(Math.random() * 20) + 70 },
        { name: 'Area C', rate: Math.floor(Math.random() * 20) + 60 },
...
>
  ...
  Generate Report
      ...
      <div className="mt-8">
        <h2 className="text-xl font-semibold mb-4 text-gray-800">Available Reports</h2>
        <ul className="space-y-2">
          <ReportItem title="Last Week's Collection Summary" date="2023-10-11" onDownload={handleDownloadExistingReport} />
          <ReportItem title="Monthly Waste Analysis" date="2023-09-30" onDownload={handleDownloadExistingReport} />
          <ReportItem title="Quarterly Financial Report" date="2023-09-30" onDownload={handleDownloadExistingReport} />
          <ReportItem title="Annual Environmental Impact Assessment" date="2022-12-31" onDownload={handleDownloadExistingReport} />
        </ul>
      </div>
    </div>
  );
